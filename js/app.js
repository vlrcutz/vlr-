    ...
    return `
      <div class="list-row">
        <div class="main">
          ...
          <div class="meta">${time} · ${s.paymentMethod}</div>
        </div>
...
async function renderCustomers() {
  ...
  const rows = filtered.map((c) => {
    ...
    return { c, visits: custSales.length, totalSpent };
  }).sort((a,b) => b.totalSpent - a.totalSpent);

  list.innerHTML = rows.map(({c, visits, totalSpent}) => `
    <div class="list-row" style="cursor:pointer;" data-cust="${c.id}">
      <div class="main"><div class="name">${escapeHtml(c.name)}</div><div class="meta">${c.phone ? escapeHtml(c.phone) + " · " : ""}${visits} visit${visits===1?"":"s"}</div></div>
...
async function showCustomerDetail(custId) {
  ...
  if (!sales.length) {
    ...
    hist.innerHTML = sales.map((s) => {
      ...
      return `<div class="list-row"><div class="main"><div class="name">${escapeHtml(s.items.map(i=>i.name).join(", "))}</div><div class="meta">${time}</div></div><div class="amt">${fmt(s.total)}</div></div>`;
    }).join("");
  }
